...
case class SSTBuilder[Σ](alphabets: Set[Σ],
  ...
  def constraintsToSST(list: List[RelCons], set: Set[RegCons[Σ]]): SST[SST_State, Σ, Int, SST_Var] = {
    ...
    def compose0[Y](sst1: SST[SST_State, Σ, Σ, SST_Var], sst2: SST[SST_State, Σ, Y, SST_Var]) = Composition.compose(sst1, sst2).rename(0).trim
  ...
  def relationalToSST(cons: RelCons): SST[SST_State, Σ, Σ, SST_Var] = {
    ...
    def _constraintToSST1(cons: TransducerConstraint[Σ]): SST[SST_State, Σ, Σ, SST_Var] = {
      ...
      val s0: SST_State = if (idx == 0) transToSST(trans.s0) else states(0)
      ...
      else Map((states(idx - 1), split) -> transToSST(trans.s0))
