...
case class Builder[Σ](alphabets : Set[Σ],
  ...
  private def _constraintToSST1(cons : TransducerConstraint[TransState, Σ]) : SST[SST_State, Σ, Σ, SST_Var] = {
    ...
    val states : List[SST_State] = List.range(0, num).filter(i=> i!=idx).map(x=> SST_State(x, sstName))
    ...
    SST(states.toSet ++ transToSST.map(x=>x._2).toSet + sink,
      s0,
      vars.toSet,
      delta.withDefaultValue(sink),
      eta.withDefaultValue(vars.map(x=> x->List[Either[SST_Var, Σ]]()).toMap),
  ...
  def rename(num : Int, sst : SST[((SST_State, Map[(SST_State, SST_Var), SST_State]), Map[(SST_State, SST_Var), Map[SST_Var, List[SST_Var]]]),
    Σ,
    Σ,
    ((SST_State, SST_Var), SST_Var, Int)]) : SST[SST_State, Σ, Σ, SST_Var]={

    val sstName = "r"+num

    val toNewState : Map[((SST_State, Map[(SST_State, SST_Var), SST_State]), Map[(SST_State, SST_Var), Map[SST_Var, List[SST_Var]]]), SST_State] =
      sst.states.toList.zipWithIndex.map(x=> x._1 -> SST_State(x._2, sstName)).toMap

    val toNewVar : Map[((SST_State, SST_Var), SST_Var, Int), SST_Var] =
      sst.vars.toList.zipWithIndex.map(x=> x._1-> SST_Var(x._2, sstName)).toMap
